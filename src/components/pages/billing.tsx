...
import { billingSummary, paymentMethodsData } from "@/lib/data";
...
const InvoiceTable = () => {
    ...
    // Filter invoices to show only those for the current firm's clients
    const clientIds = clients.map(c => c.id);
    const firmInvoices = invoicesData.filter(invoice => clientIds.includes(invoice.client.id));
        ...
        <Card>
            ...
            <CardContent>
                ...
                <div className="flex items-center justify-between mt-4">
                    <p className="text-sm text-muted-foreground">Showing 1 to {firmInvoices.length} of {firmInvoices.length} results</p>
...
const InvoicePreview = () => {
    const { invoicesData } = useGlobalData();
    const invoiceToPreview = invoicesData.find(i => i.invoiceNumber === 'INV-2023-0456');
                ...
                <div className="border rounded-xl p-8">
                    <div className="flex justify-between mb-12">
                        ...
                        <div className="text-right">
                            ...
                            <p className="mt-4">Invoice #: {invoiceToPreview.invoiceNumber}</p>
                            <p>Date: {invoiceToPreview.date}</p>
                            <p>Due Date: <span className="text-red-600">{invoiceToPreview.dueDate}</span></p>
                            <div className="mt-4">Status: <Badge variant={getStatusBadgeVariant(invoiceToPreview.status)}>{invoiceToPreview.status}</Badge></div>
                        </div>
                    </div>
                    <div className="mb-12">
                        <Table>
...
const PaymentsTable = () => {
    const { clients, invoicesData } = useGlobalData();
    const paymentsData = invoicesData.filter(i => i.status === 'Paid');
