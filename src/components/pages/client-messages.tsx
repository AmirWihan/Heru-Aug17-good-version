...
// For demo purposes, the client is James Wilson
const CURRENT_USER_NAME = 'James Wilson';
                        ...
                        <CardHeader className="flex flex-row items-center justify-between p-4 border-b">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground flex items-center gap-1">Online</p>
                                </div>
                            </div>
                                ...
                                {selectedConversation.messages.map((msg) => (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            msg.sender === CURRENT_USER_NAME ? 'ml-auto flex-row-reverse' : 'mr-auto'
                                        )}
                                    >
                                        <div
                                            className={cn(
                                                'p-3 rounded-2xl',
                                                msg.sender === CURRENT_USER_NAME
                                        ...
                                        >
                                            <p className="text-sm">{msg.text}</p>
                                             <div className={cn("flex items-center justify-end gap-1 text-xs mt-1",
                                                msg.sender === CURRENT_USER_NAME ? 'text-primary-foreground/70' : 'text-muted-foreground'
                                            )}>
                                                <span>{msg.timestamp}</span>
                                                {msg.sender === CURRENT_USER_NAME && <CheckCheck className="h-4 w-4" />}
                                            </div>
                                        </div>
                                    </div>
                                ))}
